...
const Adddoctorinfo = (props: Props) => {
  let {
    apiurl,
    userData,
    alertMSG,
  ...
  console.log(userData.current.doctorinfo);
  const [name, setName] = useState<string>(
    userData.current.doctorinfo.fullname
  );
  const [email, setEmail] = useState<string>(userData.current.doctorinfo.email);
  const [date, setDate] = useState<Date | null>(
    new Date(userData.current.doctorinfo.dob)
  );
  const [phone, setPhone] = useState<string>(userData.current.doctorinfo.phone);
  const [gender, setGender] = useState<string>(
    userData.current.doctorinfo.gender
  );
  const [title, setTitle] = useState<string>(userData.current.doctorinfo.title);
  ...
  const save = () => {
    if (
      ...
      setLoading(true);
      axios
        .patch(`${apiurl}/doctor/update/${userData.current.doctorinfo.id}`, {
          fullname: name,
          title: title,
          phone: phone,
          email: email,
